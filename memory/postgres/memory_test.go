...
func setupTestDB(t *testing.T) *PostgresMemoryService {
	...
	// Use a timeout context to prevent hanging if database is unavailable
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	svc, err := NewPostgresMemoryService(ctx, PgMemSvrConfig{
	...
	// Clean up test data
	_, err = svc.DB().
		ExecContext(context.Background(), "DELETE FROM memory_entries WHERE app_name LIKE 'test_%'")
	if err != nil {
...
func TestAddSession(t *testing.T) {
	...
	var count int
	err = svc.DB().
		QueryRowContext(ctx, "SELECT COUNT(*) FROM memory_entries WHERE app_name = 'test_app'").
		Scan(&count)
	if err != nil {
...
func TestAddSessionDuplicates(t *testing.T) {
	...
	var count int
	err = svc.DB().
		QueryRowContext(ctx, "SELECT COUNT(*) FROM memory_entries WHERE session_id = 'sess-dup'").
		Scan(&count)
	if err != nil {
		t.Fatalf("Failed to count entries: %v", err)
	}
...
func TestAddSessionEmptyEvents(t *testing.T) {
	...
	var count int
	err = svc.DB().
		QueryRowContext(ctx, "SELECT COUNT(*) FROM memory_entries WHERE session_id = 'sess-empty'").
		Scan(&count)
	if err != nil {
		t.Fatalf("Failed to count entries: %v", err)
	}
